...
import { IVideoPlayerContext } from "./usePlayer"
...
const useHotkeys = ({
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      switch (event.key) {
        ...
        case "c":
          ...
          break
        case Key.ArrowUp:
          volumeState.set((prev) => Math.min(prev + 5, 100))
          break
        case Key.ArrowDown:
          volumeState.set((prev) => Math.max(prev - 5, 0))
          break
        default:
          break
      }
  ...
  }, [onPause, onPlay, toggleCaptions, volumeState])
}
